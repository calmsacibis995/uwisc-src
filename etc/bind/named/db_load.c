#ifndef lint
static char sccsid[] = "@(#)db_load.c	4.7 (Berkeley) 9/8/86";
#endif
	...
	while ((n = gettoken(fp)) != EOF) {
		...
		fndtype:
			...
			if (zone == 0)
				...
				htp = hashtab;
			if ((n = db_update(domain, dp, dp, DB_NODATA, htp)) < 0) {
#ifdef DEBUG
	...
	FILE *fp;
{
	...
	if (debug)
		...
		ntp->my_addr.s_addr = inet_addr(tm);
		if ( ntp->my_addr.s_addr == (unsigned)-1) {
			/* resolve name to adderss - XXX */
		}
		ntp->next = NULL;
		...
		duplicate = 0;
		if (fnettab == NULL) {
			fnettab = ntip = ntp;	/* First time only */
		} else {
			/* All other times, add to linked list */
	...
	if (debug >= 2)
		for (ntp = fnettab; ntp != NULL; ntp = ntp->next) {
			...
			ntp->net, ntp->mask);
			fprintf(ddt," my_addr x%x\n", ntp->my_addr);
			fprintf(ddt,"\tdest_addr x%x", ntp->dest_addr.sin_addr);
