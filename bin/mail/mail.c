...
/*
 *  this code is lifted dfirectly from 4.2 /bin/mail.c.
 *  I am re-using it for 4.3/NFS mail programs since flock is
 *  not usable on fs machines.
 *  
 *  david parter	2/17/86
 */
...
char *file;
{
	...
	for (;;) {
		...
		time(&curtime);
#ifdef UW
		if (curtime < sbuf.st_ctime + 300) {	/* wait 5 mins */
#else
