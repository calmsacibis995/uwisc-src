	...
	for (i = 0; i < 2; i++) {
		...
		if (tp->prop_list) {
			for (op = tp->prop_list; op; op = op->next)
						(--((&_iob[1]))->_cnt >= 0 ?	(int)(*(unsigned char *)((&_iob[1]))->_ptr++ = ('\t')) :	((((&_iob[1]))->_flag & 0200) && -((&_iob[1]))->_cnt < ((&_iob[1]))->_bufsiz ?		((*((&_iob[1]))->_ptr = ('\t')) != '\n' ?			(int)(*(unsigned char *)((&_iob[1]))->_ptr++) :			_flsbuf(*(unsigned char *)((&_iob[1]))->_ptr, (&_iob[1]))) :		_flsbuf((unsigned char)('\t'), (&_iob[1])))), printsq(	(op->sqr - board), (1));
			some++;
		}
		if (!some)
			printf((&xstr[5334]));
