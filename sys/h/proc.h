...
/*
 * RCS Info	
 *	$Header: proc.h,v 3.1 86/10/22 13:25:24 tadl Exp $
 *	$Locker:  $
 */
...
#ifdef	SYSCALLMETER
	int		p_metermask;	/* bits indication the calls to be metered */
	struct	file *p_meter;	/* The descriptor of a connected socket */
	struct	mbuf *p_meterlist;	/* pointer to unsent trace messages */
	short	p_metercount;	/* count of traces queued and not sent */
#else	SYSCALLMETER
	int		p_dummy1;
	struct	file *p_dummy2;
	struct	mbuf *p_dummy3;
	short	p_dummy4;
#endif	SYSCALLMETER
};
...
/* #ifdef WISC_SHMEM */
/*
 * Local (University of Wisconsin)
 */
#define SSHMEM	0x2000000	/* process has shared memory attached */
/* #endif WISC_SHMEM */
