 ...
 */
#ifndef lint
static char rcs_id[] = {"$Header: hp.c,v 3.2 86/11/13 15:32:58 david Exp $"};
#endif not lint
/*
 * RCS Info
 *	$Locker:  $
...
struct	size {
	...
	49600,	0,		/* D=cyl 0 thru 154 */
	71040,	155,		/* E=cyl 155 thru 376 */
	71040,	377,		/* F=cyl 377 thru 598 */
	71040,	599,		/* G=cyl 599 thru 820 */
	213440,	155,		/* H=cyl 155 thru 820 */
#else
	15884,	0,		/* A=cyl 0 thru 49 */
	33440,	50,		/* B=cyl 50 thru 154 */
	263360,	0,		/* C=cyl 0 thru 822 */
...
#define SI_ECC_BUG	1
		mi->mi_flags = SI_ECC_BUG;	/* SI ECC mode */
		return (type);
	}
	...
	int flag;
{
	...
	if (bp->b_flags & B_BAD)
		...
		/* Check for SI ECC bug (from jrh@si and ber@harpo) */
		if (mi->mi_flags == SI_ECC_BUG)
			npf = ((rp->hpdc & 0xffff) * (st->nspc)) +
			      ((((rp->hpda) >> 8) & 0xff) * st->nsect) +
			      ((rp->hpda) & 0xff) - 
			      ((st->sizes[xunit].cyloff) * st->nspc) - 1;
		else {
			npf = bp->b_bcount - bcr;
			/*
