 ...
 */
#ifdef UW
/* modified by tom christiansen on Tue Jul  8 09:53:19 CDT 1986
 * to include a -p option to print file
 * names as full path names. also added
 * pointer check taken from gould source.
*/
#endif UW
...
#ifdef UW
			case 'p':       /* print full pathnames as headers */
				pflg++;
				continue;
#endif UW
...
getpath(name)
    register char *name;
{
	static char hname [ 60 ], dname [ 1025 ] ;
    register char *where,*hp,*dp;
    static char buffer[1200];
    extern char *getwd(),*index();

	hp =  (gethostname(hname,60) < 0) ? "<unknown host>" : hname;
    dp = (name && name[0] != '/') ? getwd(dname) : 0;
    if (where=index(hp,"."))
        *where = 0;
	return sprintf( buffer, "%s:%s%s%s", hp,dp?dp:"",dp?"/":"",name);
}
#endif UW
