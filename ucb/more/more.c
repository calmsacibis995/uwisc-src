...
#ifdef UW
    /* if input from a file, force screen refresh */
    if (!no_intty) {
	static char redraw_char = '\f';
	(void) ioctl(2, TIOCSTI, &redraw_char);
    }
#endif UW
}
...
int *length;
{
	...
	*p++ = c;
	if (c == '\t')
#ifdef UW
            if (clreol) {   /* always if -c is set */
                *(p-1) = ' ';
                for (column++; column & 7 && p < &Line[LINSIZ - 1]; column++) {
                    *p++ = ' ';
                }
            } else
#endif UW
