...
#ifdef UW
    if (strncpy(who,getlogin(),8) == NULL)
        strncpy(who,getpwuid(getuid())->pw_name);
    if ((mytty = ttyname(stdin)) == NULL)
        if ((mytty = ttyname(stdout)) == NULL)
            sprintf(mesg,
                "\n^G^G^GBroadcast Message from %s@%s at %d:%02d ...\r\n\n"
				, who
                , hostname
                , localclock -> tm_hour
                , localclock -> tm_min
        ...
        sprintf(mesg,
            "\n^G^G^GBroadcast Message from %s@%s (%s) at %d:%02d ...\r\n\n"
			, who
            , hostname
            , mytty
            , localclock -> tm_hour
