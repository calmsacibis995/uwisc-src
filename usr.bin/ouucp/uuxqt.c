#ifndef lint
static char sccsid[] = "@(#)uuxqt.c	5.7 (Berkeley) 10/9/85";
#endif
...
char *argv[];
{
	...
	int argnok;
	int notiflg;
	char xfile[MAXFULLNAME], user[MAXFULLNAME], buf[BUFSIZ*4];
...
doprocess:
	...
	while (gtxfile(xfile) > 0) {
		...
		badfiles = 0;
		while (fgets(buf, BUFSIZ*4, xfp) != NULL) {
			switch (buf[0]) {
			case X_USER:
