		...
		*cmd = '\0';
		...
		switch (c->cmdcode)
		{
...
#ifdef UW
			/* checking this screws up MH so we take it out */
#else
...
#ifdef UW
				/* Other method is GROSS */
				define('s', newstr(RealHostName), CurEnv);
#else
		  ...
		  case CMDMAIL:		/* mail -- designate sender */
			...
			if (RealHostName != NULL && macvalue('s', CurEnv) == NULL)
				define('s', RealHostName, CurEnv);
			define('r', "SMTP", CurEnv);    /* define receiving protocol */
			MeToo = TRUE;       /* if sender is receiver, give them mail too */
#endif UW
