#ifndef lint
static char rcs_id[] =
	{"$Header: svc_auth_unix.c,v 1.1 86/09/05 09:17:48 tadl Exp $"};
#endif not lint
/*
 * RCS Info
 *	$Locker: tadl $
	...
	register struct rpc_msg *msg;
{
	...
	if (buf != NULL) {
		...
		aup->aup_uid = IXDR_GET_U_SHORT(buf);
		aup->aup_gid = IXDR_GET_U_SHORT(buf);
		...
		aup->aup_len = gid_len;
		for (i = 0; i < gid_len; i++) {
			aup->aup_gids[i] = IXDR_GET_U_SHORT(buf);
		}
		/*
