...
#define DEBUG
#ifndef lint
static char rcs_id[] = {"$Header: uda.c,v 3.1 86/10/22 14:09:29 tadl Exp $"};
#endif not lint
/*
 * RCS Info
 *	$Locker:  $
...
#ifdef INST_DISK
	15884,	0,		/* A=blk 0 thru 15883 */
	15884,	15884,		/* B=blk 15884 thru 25649 */
	-1,	0,		/* C=blk 0 thru end */
	0,	0,		/* D=unused */
	0,	0,		/* E=unused */
	0,	0,		/* F=unused */
	-1,	31768,		/* G=blk 25650 thru end */
	0,	0,		/* H=unused */
#else
	15884,	0,		/* A=blk 0 thru 15883 */
...
#ifdef UW
			case    51:
				ra_info[ui->ui_unit].ra_sizes = rd51_sizes;
				break;
#endif
			case    52:
...
#ifdef MVAX
			case    50:
				ra_info[ui->ui_unit].ra_sizes = rx50_sizes;
				break;
#endif MVAX
			case    60:
	...
	register struct mslg *mp;
{
	...
	log(LOG_WARNING, "uda%d: %s error, ", um->um_ctlr,
		mp->mslg_flags & ( M_LF_SUCC | M_LF_CONT ) ? "soft" : "hard");
	switch (mp->mslg_format) {
	case M_FM_CNTERR:
		log(LOG_WARNING, "controller error, event 0%o\n", mp->mslg_event);
		break;

	case M_FM_BUSADDR:
		log(LOG_WARNING, "host memory access error, event 0%o, addr 0%o\n",
			mp->mslg_event, mp->mslg_busaddr);
		break;

	case M_FM_DISKTRN:
		log(LOG_WARNING, "disk transfer error, unit %d, grp 0x%x, hdr 0x%x, event 0%o\n",
			mp->mslg_unit, mp->mslg_group, mp->mslg_hdr,
mp->mslg_event);
		break;

	case M_FM_SDI:
		log(LOG_WARNING, "SDI error, unit %d, event 0%o, hdr 0x%x\n",
			mp->mslg_unit, mp->mslg_event, mp->mslg_hdr);
		for(i = 0; i < 12;i++)
			log(LOG_WARNING, "\t0x%x",mp->mslg_sdistat[i] & 0xff);
		log(LOG_WARNING, "\n");
		break;

	case M_FM_SMLDSK:
		log(LOG_WARNING, "small disk error, unit %d, event 0%o, cyl %d\n",
			mp->mslg_unit, mp->mslg_event, mp->mslg_sdecyl);
		break;

	default:
		log(LOG_WARNING, "unknown error, unit %d, format 0%o, event 0%o\n",
