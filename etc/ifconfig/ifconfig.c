...
#ifdef CRYNET
#include <netcry/cry.h>
#include <netcry/cry_if.h>
#endif CRYNET
...
#ifdef CRYNET
	{ "cry",    AF_CRY,     cry_status, cry_getaddr },
#endif CRYNET
	{ 0,		0,		0,		0 }
};
...
cry_getaddr(addr, saddr)
char *addr;
struct sockaddr *saddr;
{
	register struct sockaddr_cry *scry = (struct sockaddr_cry *)saddr;
	u_char netnum;
	char *index();
	...
	scry->scry_family = AF_CRY;
	scry->scry_addr.c_net = atoi(addr);
	scry->scry_addr.c_node = atoi(++s);

}

cry_status()
{
	struct sockaddr_cry *scry;
	char *cry_ntoa();

	close(s);
	s = socket(AF_CRY, SOCK_DGRAM, 0);
	if (s < 0) {
		/* if crystal isn't supported on this system, return quietly */
		if ( errno == EAFNOSUPPORT )
			return;
		perror("ifconfig: socket");
		exit(1);
	...
	strncpy(ifr.ifr_name, name, sizeof ifr.ifr_name);
	scry = (struct sockaddr_cry *)&ifr.ifr_addr;
	printf("%s: cry %s ", name, cry_ntoa(scry));
	printb("flags", flags, IFFBITS);
	putchar('\n');
}
	...
	struct sockaddr_cry *scry;
{
	...
	sprintf (buf, "%x.%x", scry->scry_addr.c_net, scry->scry_addr.c_node);
	return (buf);
}
#endif CRYNET
