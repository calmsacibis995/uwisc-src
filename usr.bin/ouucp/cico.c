#ifndef lint
static char sccsid[] = "@(#)cico.c	5.8 (Berkeley) 6/24/85";
#endif
...
#ifdef h_addr
			if (hp == 0) {
				/* hack to make things work when inverse queries fail */
				hp = gethostbyname(PhoneNumber);
				if(hp != 0) {
					register i;

					/* check all addresses of this host to see if any match */
...
#else
						int an_int;
						bcopy(hp->h_addr_list[i], &an_int, sizeof(int));
						if(from.sin_addr.s_addr == an_int)
							break;
#endif vax
					}
					/* if none match, assume host is bogus */
