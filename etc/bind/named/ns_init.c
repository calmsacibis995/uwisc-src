#ifndef lint
static char sccsid[] = "@(#)ns_init.c	4.8 (Berkeley) 9/15/86";
#endif
	...
	char *bootfile;
{
	...
	/* allocate file cache hash table */
	...
	top:	
		if (cistrcmp(buf, "cache") == 0) {
			...
			if (cistrcmp(buf, "primary") == 0)
				...
				type = Z_DOMAIN;
			else if (cistrcmp(buf, "sortlist") == 0) {
				get_sort_list(fp);
				continue;
			} else {
	...
	struct zoneinfo *zp;
{
	...
	for( cnt = 0; cnt < zp->z_addrcnt; cnt++) {
		...
		for (;;) {
			...
			if ((getshort(tmp)) == T_SOA) {
				if (soacnt == 0) {
				    soacnt++;
				    dn_expand(buf, buf + 512, np, name,
					sizeof(name));
				    tmp += 3 * sizeof(u_short) + sizeof(u_long);
				    tmp += dn_skip(tmp);
				    tmp += dn_skip(tmp);
				    serial = getlong(tmp);
				    continue;
				}
				dn_expand(buf, buf + 512, np, name2,
					sizeof(name2));
				if (cistrcmp(name, name2) != 0)
					continue;
				soa_zinfo(zp, tmp);
	...
	if ((error) && (!zp->z_sysloged)) {
	    syslog(LOG_WARNING, 
		"zoneinit: Can't find Master for secondary zone %s",
		zp->z_origin);
	    zp->z_sysloged++;
	}
	if (zp->z_source) {
#ifdef	DEBUG
		if (debug)
			fprintf(ddt, "loading secondary zone %s from %s\n",
			    zp->z_origin, zp->z_source);
#endif	DEBUG
		(void) db_load(zp->z_source, zp->z_origin, zp - zones);
	}
	/*
