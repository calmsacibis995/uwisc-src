	...
	char line[];
{
...
#ifdef UW
    /* don't be unix-like if obviously arpa-style */
    int comma_only = (int) index(line, ',') || (int) index(line, '<');
#endif UW
	...
	cp = line;
#ifdef UW
    while ((cp = yankword(cp, nbuf, comma_only)) != NOSTR) {
#else
	while ((cp = yankword(cp, nbuf)) != NOSTR) {
		...
		if (np != NIL && equal(nbuf, "at")) {
			strcpy(abuf, nbuf);
#ifdef UW
            if ((cp = yankword(cp, nbuf, comma_only)) == NOSTR) {
#else
 ...
 */
#ifdef UW
 /* modified by dlc to handle non-unix addressing -- word word <address>
  * is one address if there are commas
  */
char *withspace = " \t,(";
char *withoutspace = "\t,(";
#endif UW
...
#ifdef UW
    register int comma_only;
    register int quoted = 0; /* mod dlc -- see below */
    register int routaddr = 0; /* mod dlc -- see below */
#endif UW

	cp = ap;
	do {
#ifdef UW
		while (*cp && any(*cp, withspace))
			cp++;
#else
		while (*cp && any(*cp, " \t,"))
			cp++;
		...
		if (*cp == '\0')
			return(NOSTR);
#ifdef UW
    } while (any(*cp, withspace));
    /* mod dlc -- check for quoted text -- all one argument RFC822 i think */
    for (cp2 = wbuf; *cp && (!any(*cp, comma_only ? withoutspace:withspace) ||
            quoted); *cp2++ = *cp++)
        if (*cp == '"')
            quoted = !quoted;
		else if (*cp == '<') {  /* ucbmail is broken -- handle rout-addr */
            cp2 = wbuf;     /* by restarting with just rout-addr */
            cp++;           /* skip the '<' */
            routaddr++;
        } else if (routaddr && (*cp == '>')) {  /* end at end of routaddr */
            cp++;
            break;
        }
#else
